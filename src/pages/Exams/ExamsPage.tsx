...
export function ExamsPage() {
  ...
  return (
    <div className="pt-20 pb-12 px-4 max-w-7xl mx-auto" dir="rtl">
      {/* Header */}
      ...
      {!selectedYearId && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {collegeData.map((year: any) => (
            <div
              key={year.id}
              onClick={() => setSelectedYearId(year.id)}
              className="p-5 rounded-2xl bg-card border border-border/60 hover:border-primary/50 transition-all cursor-pointer group shadow-xs hover:shadow-md flex items-center gap-4"
            >
              ...
              <div>
                <h3 className="font-bold text-foreground text-sm mb-1">{getYearName(year)}</h3>
                <p className="text-xs text-muted-foreground">{year.departments?.length || 0} الأقسام المتاحة</p>
              </div>
            </div>
          ))}
        </div>
          ...
          {selectedYear?.departments?.map((dept: any) => {
            ...
            return (
              <div
                ...
                className={`p-5 rounded-2xl bg-card border border-border/60 transition-all shadow-xs flex items-center gap-4 ${
      ...
      {/* الخطوة 3: اختيار الترم (تظهر فور اختيار القسم) */}
      {selectedYearId && selectedDeptId && !selectedSemKey && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl mx-auto">
          <div
            onClick={() => setSelectedSemKey('semester1')}
            className="p-6 rounded-2xl bg-card border border-border/60 hover:border-primary/50 transition-all cursor-pointer group shadow-xs hover:shadow-md flex items-center gap-4"
          >
            <div className="w-12 h-12 rounded-xl bg-primary/10 text-primary flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
              <Layers className="w-6 h-6" />
            </div>
            <div>
              <h3 className="font-bold text-foreground text-base mb-1">الترم الأول</h3>
          ...
          >
            ...
            <div>
              <h3 className="font-bold text-foreground text-base mb-1">الترم الثاني</h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {activeCourses.map((course) => (
                <div
                  ...
                  className="p-5 rounded-2xl bg-card border border-border/60 hover:border-primary/50 transition-all cursor-pointer group shadow-xs hover:shadow-md flex items-center justify-between gap-3"
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-10 h-10 rounded-xl bg-primary/10 text-primary flex items-center justify-center shrink-0">
                      <FileCheck2 className="w-5 h-5" />
                    </div>
                    <span className="font-bold text-foreground text-sm truncate">{course.name}</span>
                  </div>
                  <span className="text-xs bg-primary/10 text-primary px-3 py-1.5 rounded-xl font-medium shrink-0 group-hover:bg-primary group-hover:text-primary-foreground transition-colors">
                    ابدأ الاختبار
                  </span>
                </div>
              ))}
