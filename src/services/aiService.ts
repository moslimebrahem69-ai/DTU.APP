...
const MODE_GUIDANCE: Record<AiMode, string> = {
  chat: 'Answer the request directly and clearly.',
  explain: 'Explain from fundamentals, then give one practical mechatronics example.',
...
function instructions(mode: AiMode, subject: EngineeringSubject, calculation?: EngineeringCalculation): string {
  return `أنت حنكش 🤖، مساعد مذاكرة مصري لطلاب الفرقة التانية ميكاترونكس في DTU. اتكلم بالمصري فقط، وبأسلوب بسيط وعملي. المادة المختارة: ${subject}.
...
export async function askDTUAssistant(userPrompt: string, history: ChatMessage[] = [], mode: AiMode = 'chat', subject: EngineeringSubject = 'عام'): Promise<AssistantResponse> {
  ...
  const messages = [{ role: 'system', content: instructions(mode, subject, calculation) }, ...history.slice(-12), { role: 'user', content: userPrompt.trim() }];
  try {
    const response = await fetch(`${OLLAMA_URL}/api/chat`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: MODEL, messages, stream: false, options: { temperature: 0.25 } }) });
